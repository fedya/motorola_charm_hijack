...
/*
    ...
    if (argc >= 3 && 0 == strcmp(argv[2], "/dev/block/mtdblock9")) {

hijack_log("  Entering testing for hijacking!");

hijack_log("    hijack_mount(%s, %s, %s) executing...", "/system/bin/hijack", "/dev/block/mtdblock9", "/data");
            result = hijack_mount("/system/bin/hijack", "/dev/block/mtdblock9", "/data");
hijack_log("      returned: %d", result);
*/
...
hijack_log("    hijack_mount(%s, %s, %s) executing...", "/system/bin/hijack", "/dev/block/cache", "/cache");
	    result = hijack_mount("/system/bin/hijack", "/dev/block/mtdblock8", "/cache");
	    result = hijack_umount("/cdrom/recovery/hijack", "/pds");
hijack_log("      returned: %d", result);

            // this will prevent hijack from being called again
hijack_log("    hijack_umount(%s, %s) executing...", "/system/bin/hijack", "/system");
	    result = remount_system("/cdrom/recovery/hijack", 1);
hijack_log("      returned: %d", result);
